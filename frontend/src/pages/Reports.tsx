...
import { getWeeklyReport, getKpiScorecard, getReportsWeekly, getPreCallBrief } from '../services/api'
import { useBrand } from '../context/BrandContext'
import { FileBarChart2, Loader2, CheckCircle, AlertTriangle, XCircle, PhoneCall } from 'lucide-react'
...
export default function Reports() {
  ...
  const loadPreCallBrief = () => {
    if (!activeBrand) return
    setLoading('brief')
    setBrief(null)
    getPreCallBrief(
      activeBrand.name,
      `Brand: ${activeBrand.name} | Industry: ${activeBrand.industry} | Goals: ${activeBrand.goals || 'lead generation'}`
    )
      .then(r => setBrief(r.data.result?.brief || r.data.result))
      .catch(() => {})
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3 flex-wrap">
        <button className="btn-primary flex items-center gap-2" onClick={loadPreCallBrief} disabled={!!loading || !activeBrand}>
          {loading === 'brief' ? <Loader2 size={14} className="animate-spin" /> : <PhoneCall size={14} />}
          Pre-Call Brief {activeBrand ? `— ${activeBrand.name}` : '(select a client)'}
        </button>
        <button className="btn-secondary flex items-center gap-2" onClick={loadWeekly} disabled={!!loading}>
      ...
      {loading && (
        <div className="card flex items-center gap-3">
          ...
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
            {loading === 'ai' ? 'A7 agent generating 8-stop narrative... (30–60s)'
            : loading === 'brief' ? 'Generating pre-call brief... (15–30s)'
            : 'Loading report...'}
          </p>
        </div>
      )}
      ...
      {brief && (
        <div className="card">
          <div className="flex items-center gap-2 mb-4">
            <PhoneCall size={14} style={{ color: 'var(--brand)' }} />
            <h3 className="text-sm font-semibold text-white">Pre-Call Brief — {activeBrand?.name}</h3>
            <span className="text-[10px] ml-auto" style={{ color: 'var(--text-hint)' }}>Generated by A7 · Read aloud in ~60s</span>
          </div>
          <div className="space-y-3">
            {brief.split('\n').filter(line => line.trim()).map((line, i) => (
              <div key={i} className="flex items-start gap-3">
                <span className="text-[10px] mt-1 font-bold shrink-0" style={{ color: 'var(--brand)' }}>0{i+1}</span>
