...
The pipeline creates its own DB session because it runs as a BackgroundTask
(the request's session is already closed by the time it executes).
"""
import asyncio
...
def _meta(result: dict) -> dict:
    """Extract model_used metadata from an agent result dict."""
    return {"model_used": result.get("model_used", "unknown")}
...
async def run_full_pipeline(brand_id: str) -> dict:
    async with AsyncSessionLocal() as db:
        ...
        await update_brand_status(brand_id, "running", db)
        ...
        try:
            ...
            # ── Step 1: Competitor + Market Research (A3 → Gemini) ────────────
            ...
            await add_approval_item(
                ...
                metadata=_meta(comp_result), db=db,
            )

            # ── Step 2: Technical Health Audit (A4 → Groq) ───────────────────
                ...
                for check in health_data.get("health_checks", []):
                    if check["status"] in ("critical", "warning"):
                        await add_approval_item(
                            ...
                            title=f"{'Critical' if check['status'] == 'critical' else 'Warning'}: {check['service']}",
                            description=check["details"],
                            recommendation=f"Fix {check['service']} — impacts conversion tracking accuracy.",
                            agent_id="a4",
                            impact="high" if check["status"] == "critical" else "medium",
                            category="technical",
                            metadata=_meta(health_result), db=db,
                        )

            # ── Step 3: Google Ads Keywords + Headlines (A2 → Groq) ──────────
            ...
            await add_approval_item(
                ...
                metadata=_meta(keyword_result), db=db,
            )
            await add_approval_item(
                brand_id=brand_id, item_type="copy",
                title="RSA Headlines & Descriptions Generated",
                description="15 headlines + 4 descriptions ready for Google Ads RSA.",
                recommendation=str(headline_result.get("result", ""))[:600],
                agent_id="a2", impact="high", category="google_ads",
                metadata=_meta(headline_result), db=db,
            )

            # ── Step 4: Ad Copy Variants (A5 → Mistral) ──────────────────────
            ...
            await add_approval_item(
                ...
                metadata=_meta(google_copy), db=db,
            )
            await add_approval_item(
                brand_id=brand_id, item_type="copy",
                ...
                metadata=_meta(meta_copy), db=db,
            )
            await add_approval_item(
                ...
                metadata=_meta(hooks), db=db,
            )

            # ── Step 5: Design Brief (A6 → Mistral) ──────────────────────────
            ...
            await add_approval_item(
                ...
                metadata=_meta(static_brief), db=db,
            )

            # ── Step 6: Meta Campaign Analysis (A1 → Groq) ───────────────────
            ...
            await add_approval_item(
                ...
                metadata=_meta(meta_analysis), db=db,
            )

            # ── Step 7: Full Report (A7 → Gemini) ────────────────────────────
            ...
            await add_approval_item(
                ...
                metadata=_meta(report), db=db,
            )

            # ── Step 8: Orchestrator Summary (A0 → Groq) ─────────────────────
            ...
            await update_brand_status(brand_id, "error", db)
